...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Fruityvice');
}
...
const tools: McpToolExport['tools'] = [
  {
    name: 'get_fruit',
    description: 'Get nutritional facts for a specific fruit. Returns calories, protein, fat, carbs, sugar, and fiber per 100g (e.g., \'apple\', \'banana\').',
    inputSchema: {
      type: 'object',
      properties: {
        name: {
  ...
  {
    name: 'list_fruits',
    description: 'List all available fruits with their nutritional profiles. Returns name, calories, protein, fat, carbs, sugar, and fiber for each.',
    inputSchema: {
      type: 'object',
      properties: {},
    },
...
async function getFruit(name: string) {
  const res = await pwFetch(`${BASE_URL}/${encodeURIComponent(name.toLowerCase())}`);
  if (!res.ok) throw await httpError(res, 'Fruityvice error');
...
async function listFruits() {
  const res = await pwFetch(`${BASE_URL}/all`);
  if (!res.ok) throw await httpError(res, 'Fruityvice error');

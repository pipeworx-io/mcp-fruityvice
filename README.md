...
`tools/list` at `https://gateway.pipeworx.io/fruityvice/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/get_fruit \
  -H 'Content-Type: application/json' \
  -d '{"name":"banana"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/get_fruit`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "fruityvice": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-fruityvice"]
    }
  }
}
```
